...
const ProfileCard :React.FC <Types>=({deleteHandler,profileData={},isExtraInfo=false})=>{
    ...
    const updateUserImage=(img)=>{
        ...
        const filename =CollectionConstant.userImageDir +"/"+ profileData?.docId+".png"
    ...
    return (
        <>
            <UploadSelection visible={dialogVisible} setVisible={setDialogVisible} handlerFile={handleFilePress} handlerCamera={handleCameraPress}/>
        <View style={[styles.card,{padding: 24, paddingTop: 24, paddingRight: 16, paddingBottom: 24, paddingLeft: 16}]}>


            <View style={{justifyContent:'center',alignItems:'center'}}>


                    <View style={{zIndex:-1,position:'relative',justifyContent:'center',alignItems:'center'}}>
                            <TouchableOpacity onPress={() => setDialogVisible(true)} style={styles.cameraButton} >
                                ...
                                <View style={{flexDirection:'row', justifyContent:'space-between'}}>
                                    <View style={{marginRight:normalize(30)}}>
                                        <TouchableOpacity onPress={() =>handleCameraPress()}>
                                            ...
                                            <Image source={cameraIcon} style={{height:17,width:17}}/>
                                            <Text style={{color:"black" ,marginTop:10}}>Camera</Text>
                                        </TouchableOpacity>
                                    ...
                                    <View style={{marginLeft:normalize(30)}}>
                                        <TouchableOpacity onPress={() =>handleFilePress()}>
                                            <Image source={cameraIcon} style={{height:17,width:17}}/>
                                            <Text style={{color:"black" ,marginTop:10}}>Gallery</Text>
                                        </TouchableOpacity>
                                    </View>
                                </View>
                            </Dialog.Content>
            ...
            {isExtraInfo &&
                  <View>
                        <Flex direction={'row'} mt={10} justify={'between'}>
                              <Text style={[styles.moreDetailsTitle,{color:colors.text}]}>{t('more-info')}</Text>
                              <TouchableOpacity onPress={()=>setToggleDetails(!toggleDetails)}>
                             <Image source={chervienRight} style={{height:24,width:24,transform:[{rotate:toggleDetails?'270deg' :'90deg'}]}}/>
